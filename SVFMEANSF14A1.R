...
options(digits=17)
vt   = read.table('vftF14A1.txt',sep = '\t')
...
for (i in 1:N){
  ...
  #write.table(all1,paste(i,"para.txt",sep = "_"),quote = F,sep = "\t")
  #write.table(all2,paste(i,"latent.txt",sep = "_"),quote = F,sep = "\t")
  g[,i]  = geweke.diag(draws$para[[1]][,-4])$z
  name   = sprintf('svfdraws%d.txt',i)
  #	write(t(all),file=name,ncolumn=dim(all)[2])
}
out = rbind(m,g) #include Geweke statistics
write(t(out),file='svfmeansF14A1.txt',ncolumn=dim(out)[2])
write.table(m,paste("m.txt",sep = "_"),quote = F,sep = "\t")